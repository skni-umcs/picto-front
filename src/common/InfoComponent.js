...
export function InfoComponent({userId, generation, waitMs = 0, enableTimer=false}) {
  ...
  return (
      <Box className="infoComponent">
        ...
        {enableTimer && <Box className="infoText">Czas: {new Date(timeLeft).getMinutes()}:{new Date(timeLeft).getSeconds()}</Box>}
